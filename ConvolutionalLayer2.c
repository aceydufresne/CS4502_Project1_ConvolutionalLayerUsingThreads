...
void *processer1(void *arg);
void *processer2(void *arg);
void *processer3(void *arg);
...
char fileName[20];
printf("1");
printf("Enter an Image file pathways:");
...
float thresh = kevin->threshold;
kevin->channels = 1;
kevin->img = grey;
data.threshold = sum/length;

data.threshold = thresh + 150;
if(data.threshold > 255){
    data.threshold = 255;
}
printf("1");
return;
...
for(int i = 0; i < 4; i++){
    ...
    pthread_create(&threads[4], NULL, processer, (void *)threadInfo);
    pthread_create(&threads[5], NULL, processer1, (void *)threadInfo);
    pthread_create(&threads[6], NULL, processer2, (void *)threadInfo);
    pthread_create(&threads[7], NULL, processer3, (void *)threadInfo);
...
void *processer(void *arg){
    //pause until lock is released
    threadData *data = (threadData *)arg;
    for (int i = 0; i < data->height; i++) {
        for (int j = 0; j < data->width; j++) {
            int index = i * data->width + j;
            if (data->quadrant1[index] != 0) {
                data->quadrant1[index] = 255;
            }else{
                data->quadrant1[index] = 0;
            }
